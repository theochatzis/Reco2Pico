...
def book_profile1d(booked, df, model, xcol, ycol):
    result = df.Profile1D(model, xcol, ycol)
    booked.append(result)
    return result


def book_histo2d(booked, df, model, xcol, ycol):
    result = df.Histo2D(model, xcol, ycol)
    booked.append(result)
    return result
...
def main():
    ...
    print("[info] building RDataFrame graph")
    df = ROOT.RDataFrame(args.tree, args.input)
    if args.max_events > 0:
        # Use rdfentry_ instead of Range so this also works with implicit MT.
        df = df.Filter("rdfentry_ < {}".format(int(args.max_events)), "max-events")

    if actual_npv_branch:
        df = df.Define("rdf_npv", "static_cast<int>({})".format(actual_npv_branch))
    else:
        df = df.Define("rdf_npv", "-1")

    # Common eta reductions. All of these return one value per event per etaBin.
    df = df.Define("rdf_eta_keys", "pf_rdf::eta_keys({})".format(b_eta_bin))
    df = df.Define("rdf_eta_x", "pf_rdf::eta_centers_by_key({}, {})".format(b_eta_bin, b_eta))
    df = df.Define("rdf_eta_x_plot", "pf_rdf_runtime::eta_plot_x_by_key(rdf_eta_keys)")
    df = df.Define("rdf_abs_eta_keys", "pf_rdf::abs_eta_keys({})".format(b_eta_bin))
    df = df.Define("rdf_abs_eta_x", "pf_rdf::abs_eta_centers_by_key({}, {})".format(b_eta_bin, b_eta))
    ...
    for flavor, _ in available_flavors:
        ...
        df = df.Define("rdf_eta_rho_{}".format(flc), "pf_rdf::eta_mean_by_key({}, {})".format(b_eta_bin, rb))
        df = df.Define("rdf_eta_n_{}".format(flc), "pf_rdf::eta_sum_by_key({}, {})".format(b_eta_bin, nb))
        df = df.Define("rdf_eta_sumPt_{}".format(flc), "pf_rdf::eta_sum_by_key({}, {})".format(b_eta_bin, sb))
        df = df.Define("rdf_eta_occupancy_{}".format(flc), "pf_rdf::eta_occupancy_by_key({}, {})".format(b_eta_bin, nb))
        df = df.Define("rdf_abs_rho_{}".format(flc), "pf_rdf::abs_eta_mean_by_key({}, {})".format(b_eta_bin, rb))
    ...
    if args.progress:
        add_rdf_progress_bar(df)

    dfs_by_cat = {}
    for vb in vertex_bins:
        if vb.inclusive:
            dfs_by_cat[vb.name] = df
        else:
            dfs_by_cat[vb.name] = df.Filter(vb.filter_expression("rdf_npv"), "nPV category {}".format(vb.name))

    booked = []
    eta_profiles = {vb.name: {q: {} for q in quantities} for vb in vertex_bins}
    eta_contrib_profiles = {vb.name: {} for vb in vertex_bins}
    phi_profiles = {vb.name: {} for vb in vertex_bins}
    phi_contrib_profiles = {vb.name: {} for vb in vertex_bins}
    h2_rho_vs_npv = {}
    prof_rho_vs_npv = {}
    prof_rho_contrib_vs_npv = {}
    ...
    for vb in vertex_bins:
        ...
        for quantity in quantities:
            for flavor, _ in available_flavors:
                ...
                eta_profiles[cat][quantity][flavor] = book_profile1d(booked, df_cat, model, "rdf_eta_x_plot", ycol)
        ...
        for flavor, _ in stack_flavors:
            ...
            model = profile_model_variable(hname, htitle, eta_edges)
            eta_contrib_profiles[cat][flavor] = book_profile1d(booked, df_cat, model, "rdf_eta_x_plot", ycol)
    ...
    if not args.no_phi_plots:
        ...
        for vb in vertex_bins:
            cat = vb.name
            df_cat = dfs_by_cat[cat]
            phi_profiles[cat] = {eb: {q: {} for q in quantities} for eb in eta_bins_to_plot}
            phi_contrib_profiles[cat] = {eb: {} for eb in eta_bins_to_plot}
            ...
            for eb in eta_bins_to_plot:
                ...
                xcol = "rdf_phi_x_eta_{}_{}".format(eb_col, safe_col(cat))
                ...
                for quantity in quantities:
                    for flavor, _ in available_flavors:
                        ...
                        ycol = "rdf_phi_{}_{}_eta_{}_{}".format(quantity, flc, eb_col, safe_col(cat))
                        ...
                        phi_profiles[cat][eb][quantity][flavor] = book_profile1d(booked, df_phi, model, xcol, ycol)

                for flavor, _ in stack_flavors:
                    flc = safe_col(flavor)
                    ycol = "rdf_phi_rhoContribution_{}_eta_{}_{}".format(flc, eb_col, safe_col(cat))
                    ...
                    model = profile_model_variable(hname, htitle, phi_edges)
                    phi_contrib_profiles[cat][eb][flavor] = book_profile1d(booked, df_phi, model, xcol, ycol)
    ...
    if not args.no_rho_npv_2d and actual_npv_branch:
        ...
        for flavor, _ in available_flavors:
            ...
            for eta_key in eta_keys_for_npv:
                ...
                rho_sel_col = "rdf_rho_npv_{}_{}_{}".format(flc, label, etac)
                npv_vec_col = "rdf_npv_vec_{}_{}_{}".format(flc, label, etac)
                df_sel = df.Define(rho_sel_col, "pf_rdf::select_by_key({}, {}, {})".format(key_col, source_val_col, int(eta_key))) \
                           .Define(npv_vec_col, "pf_rdf::fill_like({}, static_cast<double>(rdf_npv))".format(rho_sel_col))
                hname = "{}_rho_vs_npvsGood_{}_{}_{}".format(prefix, flavor, label, safe_name(eta_key))
                ...
                h2 = book_histo2d(booked, df_sel, h2model, rho_sel_col, npv_vec_col)
                h2_rho_vs_npv.setdefault(flavor, {})[eta_key] = h2

                pname = "{}_directProfile_meanRho_vs_npvsGood_{}_{}_{}".format(prefix, flavor, label, safe_name(eta_key))
                ptitle = ";PV_{npvsGood};#LT#rho#GT [GeV]"
                pmodel = (pname, ptitle, args.rho_npv_bins_npv, args.rho_npv_npv_min, args.rho_npv_npv_max)
                prof_rho_vs_npv.setdefault(flavor, {})[eta_key] = book_profile1d(booked, df_sel, pmodel, npv_vec_col, rho_sel_col)
        ...
        for flavor, _ in stack_flavors:
            ...
            for eta_key in eta_keys_for_npv:
                etac = safe_col(eta_key)
                contrib_sel_col = "rdf_rho_contrib_npv_{}_{}_{}".format(flc, label, etac)
                npv_vec_col = "rdf_npv_vec_contrib_{}_{}_{}".format(flc, label, etac)
                df_sel = df.Define(contrib_sel_col, "pf_rdf::select_by_key({}, {}, {})".format(key_col, source_val_col, int(eta_key))) \
                           .Define(npv_vec_col, "pf_rdf::fill_like({}, static_cast<double>(rdf_npv))".format(contrib_sel_col))
                pname = "{}_directProfile_meanRhoContribution_vs_npvsGood_{}_{}_{}".format(prefix, flavor, label, safe_name(eta_key))
                ptitle = ";PV_{npvsGood};#LT#rho#GT contribution [GeV]"
                pmodel = (pname, ptitle, args.rho_npv_bins_npv, args.rho_npv_npv_min, args.rho_npv_npv_max)
                prof_rho_contrib_vs_npv.setdefault(flavor, {})[eta_key] = book_profile1d(booked, df_sel, pmodel, npv_vec_col, contrib_sel_col)

    run_booked_actions(booked)
